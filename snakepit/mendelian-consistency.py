...
def run_statistics(vcf_file, ped_file, samples, table_tsv, column_prefix):
	...
	with open(table_tsv, 'w') as out_tsv:
		...
		for record in vcf.Reader(filename=vcf_file):
			...
			assert len(record.INFO['ID']) == 1
			variant_id = record.INFO['ID'][0]
			total_trios = 0
			untyped_trios = 0
			if 'X' in record.CHROM or 'Y' in record.CHROM:
				continue
			vartype = determine_pyvcf_type(record)
			assert isinstance(vartype, VariantType), 'Unexpected return value: {}'.format(type(vartype))
			untyped = False
			for name in trios:
				gt_child = genotype_from_string(record.genotype(trios[name][0])['GT'])
				gt_parent1 = genotype_from_string(record.genotype(trios[name][1])['GT'])
				gt_parent2 = genotype_from_string(record.genotype(trios[name][2])['GT'])
...
#			continue
		...
		if all_consistent or sex_chrom:
			...
			if not check_all_absent(fields[9:]):
				if check_all_absent(only_parents[9:]):
					continue
				#sys.stderr.write(' '.join(fields[9:]))
				assert (not check_all_absent(only_parents[9:]))
				#sys.stderr.write('post assert')
			if remove_children:
