...
class HaplotypeTable:
	...
	def merge(self, reference, max_edit_distance):
		"""
...
def simplify_allele_IDs(alleles):
	temp_alleles = {}
	for index,ID in enumerate(alleles.keys()):
		temp_alleles[index] = alleles[ID]
	return temp_alleles
...
def run_merge(reference, vcf, ploidy, chromosomes=None,max_edit_distance=0.0):
	...
	for line in open(vcf, 'r'):
		...
		if ((start >= prev_end) or (prev_chrom != chromosome)) and not haplotypes.is_empty():
			...
			vcf_line, written = haplotypes.merge(ref,max_edit_distance)
...
if __name__ == '__main__':
	...
	parser_merge.add_argument('-max_edit_distance', metavar='edit_threshold', default=0.0, type=float, help='TODO')
	...
	if args.subparser_name == 'merge':
		...
		run_merge(args.r, args.vcf, args.ploidy, chromosomes, args.max_edit_distance)
