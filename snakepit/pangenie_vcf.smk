...
rule align_assemblies_paf:
    ...
    shell:
        '''
        paftools.js sam2paf {input} | sort -k6,6 -k8,8n > {output}
        '''
...
rule collect_all_haplotypes:
    ...
    resources:
        mem_mb=1000
    shell:
        "bcftools merge --threads {threads} -m none --missing-to-ref {input.vcfs} | python3 {scripts}/assign-variant-ids.py > {output}"
...
rule set_to_missing:
    ...
    resources:
        ...
        walltime = '24:00'
    shell:
        "python3 {scripts}/set-to-missing.py -v {input.vcf} -m {frac_missing} -f {input.bed} > {output}"
